...
def popupmsg(data):
    ...
    popup = Tk()
    popup.wm_title("Add data to database")
    label = ttk.Label(popup, text="Select age range")
    label.grid(row=0, column=0)
    B1 = ttk.Button(popup, text="Under 14", command = lambda: add_data(0, data))
    B1.grid(row=1, column=0)
    B2 = ttk.Button(popup, text="15-20", command = lambda: add_data(1, data))
    B2.grid(row=1, column=1)
    B2 = ttk.Button(popup, text="21-30", command = lambda: add_data(2, data))
    B2.grid(row=1, column=2)
    B2 = ttk.Button(popup, text="31-40", command = lambda: add_data(3, data))
    B2.grid(row=1, column=3)
    B2 = ttk.Button(popup, text="41-50", command = lambda: add_data(4, data))
    B2.grid(row=1, column=4)
    B2 = ttk.Button(popup, text="Over 50", command = lambda: add_data(5, data))
    B2.grid(row=1, column=5)
    popup.mainloop()
    
def add_data(age, data):
    global big_df
    global new_df
    global ages_count
    ...
    data['Age'] = age_dict[age]
    data = data.fillna(0)
    big_df = big_df.append(data)
    new_df = new_df.append(data)
    ages_count.loc[age_dict[age]]['new'] += 1
    ...
    popup.destroy()
    comp_popup_msg(age_dict[age], data)
         

def comp_popup_msg(age, data):
    global comp_popup
    
    
    comp_popup = Tk()
    comp_popup.wm_title("Compare data to others in age range")
    
      
    label = ttk.Label(comp_popup, text=age)
    label.grid(row=0, column=0)
    
    boxfig = Figure() 
     
    boxax = boxfig.add_subplot(111) 

    boxgraph = FigureCanvasTkAgg(boxfig, master=comp_popup) 
    boxgraph.get_tk_widget().grid(row=1, column=0)
    
    age_data = big_df[:-1]
    
    age_data = age_data.loc[age_data['Age']==age]    
    
    age_data.boxplot(column=['Mean pitch'], ax=boxax)
    
    boxax.scatter(1, data['Mean pitch'], c='red')
    
    b = Button(comp_popup, text="Predict", command= lambda: pred_audio(data, age), bg="red", fg="white") 
    b.grid(row=2, column=0)
    
    comp_popup.mainloop()

def pred_audio(data, age):
    global pred_popup
    global model
    global acc_df
    global accuracies
    ...
    pred_popup = Tk()
    pred_popup.wm_title("Predict age range")
    ...
    lbl_text1 = "Actual age range: " + age
    lbl_text2 = "Predicted age range: " + pred_age[0]
    
    label1 = ttk.Label(pred_popup, text=lbl_text1)
    label1.grid(row=0, column=0)
    
    label2 = ttk.Label(pred_popup, text=lbl_text2)
...
b3 = Button(tab1, text="Add", command= lambda: popupmsg(feats1), bg="blue", fg="white", font=f) 
b3.grid(row=5, column=0)

b4 = Button(tab1, text="Add", command= lambda: popupmsg(feats2), bg="blue", fg="white", font=f) 
...
slider = ttk.Scale(f_frame, from_=10, to=200, orient='horizontal', command=get_current_value, variable=current_value, length=400)
...
v_slider = ttk.Scale(v_frame, from_=1, to=0, orient='vertical', command=get_current_value, variable=volume, length=300)
v_slider.pack()
v_slider.set(0.5)
...
p_frame = LabelFrame(guess_frame, text="Computer thinks:", width=500, height=200, font=f, bg='white')
...
h_frame = LabelFrame(guess_frame, text="Human thinks:", width=500, height=200, font=f, bg='white')
...
a_frame = LabelFrame(guess_frame, text="Actual age:", width=500, height=200, font=f, bg='white')
...
results_frame = LabelFrame(tab4, text="Leader board", width=250, height=250, font=f, bg='white')
...
p_frame_sex = LabelFrame(guess_frame_sex, text="Computer thinks:", width=500, height=200, font=f, bg='white')
...
h_frame_sex = LabelFrame(guess_frame_sex, text="Human thinks:", width=500, height=200, font=f, bg='white')
...
a_frame_sex = LabelFrame(guess_frame_sex, text="Actual gender:", width=500, height=200, font=f, bg='white')
...
results_frame_sex = LabelFrame(tab5, text="Leader board", width=250, height=250, font=f, bg='white')
